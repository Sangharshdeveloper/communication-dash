...
.btn-green{background:#e8f5ee;color:#006B3C;border:1px solid #bbf7d0}
.btn-green:hover{background:#d1fae5;transform:translateY(-1px)}
.btn-blue{background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe}
.btn-blue:hover{background:#dbeafe;transform:translateY(-1px)}
.btn-amber{background:#fffbeb;color:#d97706;border:1px solid #fde68a}
.btn-amber:hover{background:#fef3c7;transform:translateY(-1px)}
.btn-red{background:#fef2f2;color:#dc2626;border:1px solid #fecaca}
.btn-red:hover{background:#fee2e2;transform:translateY(-1px)}
.btn-gray{background:#f3f4f6;color:#374151;border:1px solid #e5e7eb}
.btn-gray:hover{background:#e5e7eb;transform:translateY(-1px)}
...
.modal{
  ...
  animation:mIn .2s ease;overflow:hidden;
...
.fg input,.fg select{
  ...
  font-family:inherit;background:#fff;transition:all .15s;
}
.fg input:focus,.fg select:focus{
  outline:none;border-color:#006B3C;
  box-shadow:0 0 0 3px rgba(0,107,60,.1);
}
.fg .hint{font-size:11px;color:#9ca3af;margin-top:4px}
.fg .err-msg{font-size:12px;color:#dc2626;margin-top:4px}
.fg input.invalid,.fg select.invalid{border-color:#dc2626}

.status-switch{
  display:flex;align-items:center;gap:8px;cursor:pointer;
  padding:8px 0;
}
.status-switch input[type=checkbox]{width:16px;height:16px;accent-color:#006B3C;cursor:pointer}

/* Link box */
.link-box{
  background:#f9fafb;border:1.5px solid #e5e7eb;border-radius:10px;
  padding:12px 14px;font-size:13px;word-break:break-all;
  color:#374151;font-family:monospace;line-height:1.55;margin:12px 0;
}

.info-row{display:flex;align-items:center;gap:10px;font-size:13px;color:#4b5563;margin:6px 0}
.info-row strong{color:#111827;min-width:80px}
...
.alert-red{
  ...
  padding:10px 14px;font-size:12px;color:#991b1b;margin:10px 0;
}
.alert-green{
  background:#ecfdf5;border:1px solid #a7f3d0;border-radius:8px;
  padding:10px 14px;font-size:12px;color:#065f46;margin:10px 0;
}
...
.session-item:hover{border-color:#006B3C}
.sess-av{
  width:36px;height:36px;border-radius:50%;background:#006B3C;
  display:flex;align-items:center;justify-content:center;
  color:#fff;font-weight:700;font-size:13px;flex-shrink:0;
}
.sess-info{flex:1;min-width:0}
.sess-name{font-size:13px;font-weight:600;color:#111827}
.sess-meta{font-size:11px;color:#6b7280;margin-top:2px}
.open-chat-btn{
  font-size:11px;color:#006B3C;font-weight:700;text-decoration:none;
  padding:4px 10px;background:#e8f5ee;border:1px solid #bbf7d0;border-radius:6px;
  white-space:nowrap;
}
...
.add-user-btn:hover{transform:translateY(-1px);box-shadow:0 4px 12px rgba(0,107,60,.35)}
...
.pwd-match-msg.ok  { color: #059669; }
.pwd-match-msg.bad { color: #dc2626; }
...
<div class="modal-backdrop" id="chat-link-modal" onclick="closeModal('chat-link-modal')">
  <div class="modal" onclick="event.stopPropagation()">
    ...
    <div class="modal-footer">
      ...
      <button class="btn btn-outline" onclick="copyChatLink()" id="cl-copy-btn" style="display:none">📋 Copy</button>
...
<div class="modal-backdrop" id="sessions-modal" onclick="closeModal('sessions-modal')">
  <div class="modal" onclick="event.stopPropagation()">
    ...
    <div class="modal-body">
      <div id="ss-loading" style="text-align:center;padding:30px"><div class="spin-sm"></div></div>
      <div id="ss-list"></div>
...
function openUserModal(userId = null) {
  ...
  // Reset all fields
  ['user-id','user-name','user-email','user-phone','user-password','user-password-confirm','user-bitrix-agent-id']
  ...
  if (userId) {
    ...
    fetch(`${URLS.user}/${userId}`, { headers: { 'Accept': 'application/json' } })
      .then(r => r.json())
      .then(u => {
...
function onRoleChange() {
  ...
  if (role === 'customer') {
    ...
    roleInfo.innerHTML  = `ℹ️ <strong>${role.charAt(0).toUpperCase()+role.slice(1)}</strong> accounts sign in with email + password.`;
    roleInfo.className  = 'alert-info';
  }
}
...
function resetStrengthBars() {
  [1,2,3,4].forEach(i => {
    ...
    if (b) { b.className = 'pwd-strength-bar'; }
  });
}
...
async function submitUserForm() {
  ...
  if (role !== 'customer') {
    if (!activeUserId && !pwd) {
      document.getElementById('err-password').textContent = 'Password is required for new staff accounts.';
  ...
  try {
    ...
    if (res.status === 422) { showErrors(data.errors || {}); throw new Error('Validation failed'); }
...
async function refreshChatLinkPreview() {
  ...
  try {
    const res = await fetch(URLS.chatLink, {
      ...
      body: JSON.stringify({ customer_id: activeCustomerId, agent_id: activeAgentId }),
    });
    const data = await res.json();
    if (!res.ok) throw new Error(data.message);
...
async function openAgentSessionsModal(agentId, agentName) {
  ...
  try {
    const res      = await fetch(URLS.agentSess(agentId), { headers: { 'Accept': 'application/json' } });
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/u804993635/domains/imagespark.in/public_html/a/resources/views/admin/users.blade.php ENDPATH**/ ?>