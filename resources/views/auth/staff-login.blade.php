...
<div class="card">
  ...
  <div class="card-body">
    ...
    <div class="panel active" id="panel-staff">
      ...
      @if($errors->hasAny(['email','password']) && old('_form') === 'staff')
        <div class="alert alert-error">
          <span>⚠️</span><div>{{ $errors->first() }}</div>
        </div>
      @endif
      ...
      <form method="POST" action="{{ route('staff.login') }}" id="staff-form">
        @csrf
        <input type="hidden" name="_form" value="staff">

        <div class="field">
          <label>Email Address</label>
          <div class="input-wrap {{ $errors->has('email') && old('_form') === 'staff' ? 'has-error' : '' }}">
            <span class="icon">✉️</span>
            <input type="email" name="email"
              value="{{ old('_form') === 'staff' ? old('email') : '' }}"
              placeholder="you@insurance.ae"
              autocomplete="email" required>
          </div>
          @if($errors->has('email') && old('_form') === 'staff')
        ...
        <div class="field">
          ...
          <div class="input-wrap {{ $errors->has('password') && old('_form') === 'staff' ? 'has-error' : '' }}">
          ...
          @if($errors->has('password') && old('_form') === 'staff')
    ...
    <div class="panel" id="panel-customer">
      ...
      @if($errors->has('email') && old('_form') === 'customer')
        <div class="alert alert-error">
          <span>⚠️</span><div>{{ $errors->first('email') }}</div>
        </div>
      @endif

      @if(session('magic_sent'))
        <div class="alert alert-success">
          <span>✅</span><div>{{ session('magic_sent') }}</div>
        </div>
      @endif
      ...
      <form method="POST" action="{{ route('auth.magic.request') }}" id="customer-form">
        @csrf
        ...
        <div class="field">
          ...
          <div class="input-wrap">
            <span class="icon">✉️</span>
            <input type="email" name="email"
              value="{{ old('_form') === 'customer' ? old('email') : '' }}"
...
(function() {
  var form = '{{ old("_form") }}';
  var hasMagicSent  = {{ session('magic_sent') ? 'true' : 'false' }};
  var hasLinkError  = {{ $errors->has('link') ? 'true' : 'false' }};
