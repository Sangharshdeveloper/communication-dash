          ...
          <a href="{{ route('direct-chat.agent.session', $sess->id) }}"
            ...
            <div class="sess-body">
              ...
              <div class="sess-footer">
                @if($sess->customer_ref)
                  <span class="sess-ref">{{ $sess->customer_ref }}</span>
                @else
                  <span></span>
                @endif
                @if($sess->unread_count > 0)
                  <span class="unread-dot">{{ $sess->unread_count > 9 ? '9+' : $sess->unread_count }}</span>
                @endif
              </div>
            </div>
          </a>
  ...
  @forelse($messages as $msg)
    ...
    @else
     <!--<div class="msg-wrap {{ $msg['mine'] ? 'mine' : '' }}" id="m-{{ $msg['id'] }}">-->
         <div class="msg-wrap {{ ($msg['mine'] || (isset($msg['sender_role']) && $msg['sender_role'] === 'agent')) ? 'mine' : '' }}" id="m-{{ $msg['id'] }}">
  @if(!$msg['mine'])
...
@endif
    ...
    <div class="info-col">
      ...
      <div class="info-actions">
        ...
        <button class="info-btn" onclick="window.open('mailto:{{ $session->customer->email }}')">✉️ Email</button>
      </div>
      ...
      <div class="info-section">
        <h4>Bitrix Deal</h4>
        <div class="link-copy-block"><a href="{{ $session->bitrix_deal_link ?? ''}}" target="_blank" rel="noopener noreferrer">{{ $session->bitrix_deal_link ?? ''}}</a></div>
      </div>
      ...
      <div class="info-section">
        ...
        <div class="link-copy-block">{{ url('/c/'.$session->agent_id) }}?cid={{ urlencode($session->customer_ref ?? '') }}&token={{ $session->session_token }}</div>
