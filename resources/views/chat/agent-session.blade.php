...
@push('styles')
    ...
    ═══════════════════════════════════════════════════════════════ */
        ...
        .session-card.has-unread:not(.active-session) {
    ...
    ═══════════════════════════════════════════════════════════════ */
        ...
        .send-fab:hover {
            ...
            box-shadow: 0 4px 14px rgba(0, 135, 90, .45);
        }
...
@section('content')
        ...
        <div class="chat-shell">
            ...
            <div class="sessions-col">
                ...
                <div class="sessions-hdr">
                    <h3>Conversations</h3>
                    @php $totalUnreadSidebar = $allSessions->sum('unread_count'); @endphp
                    @if ($totalUnreadSidebar > 0)
                        <span class="unread-pill">{{ $totalUnreadSidebar }} unread</span>
                    @endif
                </div>
                ...
                <div class="sessions-scroll">
                    @forelse($allSessions as $sess)
                        <a href="{{ route('direct-chat.agent.session', $sess->id) }}"
                            class="session-card
                    {{ $sess->id === $session->id ? 'active-session' : '' }}
                    {{ $sess->unread_count > 0 ? 'has-unread' : '' }}"
                            data-search="{{ strtolower($sess->customer->name . ' ' . $sess->customer_ref) }}">

                            <div class="sess-av">{{ strtoupper(substr($sess->customer->name, 0, 1)) }}</div>

                            <div class="sess-body">
                                <div class="sess-top">
                                    <span class="sess-name">{{ $sess->customer->name }}</span>
                                    <span class="sess-time">
                                        {{ $sess->last_activity_at?->diffForHumans(null, true) ?? $sess->created_at->diffForHumans(null, true) }}
                                    </span>
                                </div>

                                <div class="sess-preview">
                                    @if ($sess->lastMsg)
                                        @if ($sess->lastMsg->sender_id === $sess->agent_id)
                                            <span style="color:var(--text3)">You: </span>
                                        @endif
                                        @if ($sess->lastMsg->type === 'attachment')
                                            📎 Attachment
                                        @else
                                            {{ Str::limit($sess->lastMsg->body, 40) }}
                                        @endif
                                    @else
            ...
            <div class="chat-col">
                ...
                {{-- Header --}}
                <div class="chat-hdr">
                    <a href="{{ route('direct-chat.agent.inbox') }}" class="hdr-back" title="Back to inbox">←</a>
                    ...
                    @forelse($messages as $msg)
                        ...
                        @if ($msg['date'] !== $lastDate)
                            <div class="day-divider">
                                <span>{{ \Carbon\Carbon::parse($msg['date'])->isToday() ? 'Today' : (\Carbon\Carbon::parse($msg['date'])->isYesterday() ? 'Yesterday' : \Carbon\Carbon::parse($msg['date'])->format('D, M j, Y')) }}</span>
                            </div>
                        ...
                        @else
                            ...
                            <div class="msg-wrap {{ $msg['mine'] || (isset($msg['sender_role']) && $msg['sender_role'] === 'agent') ? 'mine' : '' }}"
                                id="m-{{ $msg['id'] }}">
                                @if (!$msg['mine'])
                                    <div class="msg-av">{{ $msg['initial'] ?? '?' }}</div>
                                @endif
                                <div class="msg-grp">
                                    {{-- Sender name label — shown for ALL messages including agent's own --}}
                                    <div class="msg-sender-label {{ $msg['mine'] ? 'label-mine' : '' }}">
                                        {{ $msg['sender_name'] }}
                                        @if (isset($msg['sender_role']) && $msg['sender_role'] === 'agent')
                                    ...
                                    <div class="bubble">
                                        ...
                                        @foreach ($msg['attachments'] as $att)
                                            @if ($att['is_image'])
                                                <img src="{{ $att['url'] }}" class="att-img"
                                                    onclick="lightbox('{{ $att['url'] }}')" alt="{{ $att['name'] }}">
                                            @else
                                                <div class="att-pill" onclick="window.open('{{ $att['url'] }}')">
                                                    <span
                                                        class="att-icon">{{ str_contains($att['mime'], 'pdf') ? '📄' : '📎' }}</span>
                                                    <div>
                                        ...
                                        <div class="bubble-footer">
                                            ...
                                            @if ($msg['mine'])
                                                <span class="ticks {{ $msg['is_read'] ? 'read' : '' }}">
                                                    {{ $msg['is_read'] ? '✓✓' : '✓' }}
                                                </span>
                                            @endif
                                        </div>
                ...
                @if (!isset($isAdminView) || !$isAdminView)
                    <div class="chat-input-bar">
                        ...
                        <div class="input-grp">
                            <div class="input-box">
                                <textarea id="agent-input" rows="1" placeholder="Type a message…" oninput="resizeInput(this)"
                                    onkeydown="onKey(event)" maxlength="4000"></textarea>
                ...
                @else
                    <div
                        style="padding:10px 16px;background:#fff;border-top:1px solid #e8eaed;text-align:center;font-size:12px;color:#9ca3af;">
            ...
            <div class="info-col">
                ...
                <div class="info-profile">
                    <div class="info-big-av">{{ strtoupper(substr($session->customer->name, 0, 1)) }}</div>
                    <div class="info-cust-name">{{ $session->customer->name }}</div>
                ...
                <div class="info-section">
                    ...
                    <div class="info-item">
                        <span class="info-icon"></span>
                        <div class="info-body">
                            <small>Bitrix Deal Id</small>
                            {{ $session->customer->bitrix_deal_id ?? '—' }}
                        </div>
                    ...
                    <div class="info-item">
                        <span class="info-icon"></span>
                        <div class="info-body">
                            <small>Bitrix Contact Id</small>
                            {{ $session->customer->bitrix_contact_id ?? '—' }}
                        </div>
                    </div>

                    @if (!empty($session->bitrix_deal_link))
                        <div class="link-copy-block"><a href="{{ $session->bitrix_deal_link ?? '' }}"
                                          target="_blank" rel="noopener noreferrer">{{ $session->bitrix_deal_link ?? '' }}</a>
                        </div>
                    @endif
                    ...
                    <div class="info-section">
                        ...
                        <div class="link-copy-block">
                            {{ url('/c/' . $session->agent_id) }}?cid={{ urlencode($session->customer_ref ?? '') }}&token={{ $session->session_token }}
                        </div>
    ...
    @push('scripts')
       ...
       <span class="remove" onclick="removeFile(${i})">✕</span>
            ...
            const INBOX_URL = '{{ route('direct-chat.agent.inbox') }}';
            const CURRENT_SID = {{ $session->id }};
