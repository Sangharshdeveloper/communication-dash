  ...
  <div class="inbox-wrap">
    ...
    <div class="sessions-list">
      <div class="sessions-header">
        <h3>Conversations</h3>
        @if($totalUnread > 0)
          <span class="badge badge-success">{{ $totalUnread }} unread</span>
        @endif
      </div>
      ...
      <div class="sessions-scroll" id="sessions-scroll">
        @forelse($sessions as $session)
          <a href="{{ route('direct-chat.agent.session', $session->id) }}"
            ...
            <div class="sess-body">
              <div class="sess-top">
                <div class="sess-name">{{ $session->customer->name }}</div>
                <div class="sess-time">
              ...
              <div class="sess-preview">
                @if($session->last_msg)
                  ...
                  @if($session->last_msg->type === 'attachment')
                    📎 Attachment
                  @else
                    {{ Str::limit($session->last_msg->body, 40) }}
                  @endif
                @else
