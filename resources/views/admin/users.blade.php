                    ...
                    <div style="flex:1;min-width:140px;">
                        ...
                        <select name="role" class="form-control">
                            ...
                            @foreach (['admin', 'agent', 'auditor', 'customer'] as $role)
                                <option value="{{ $role }}" @selected(request('role') === $role)>{{ ucfirst($role) }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                @else badge-gray @endif">
                                ...
                                <td>
                                    <div class="actions-cell">
                                        <button class="action-btn btn-gray" onclick="openUserModal({{ $user->id }})"
                                            title="Edit">✎ Edit</button>

                                        @if ($user->role === 'customer')
                                            <button class="action-btn btn-blue"
                                                onclick="openMagicLinkModal({{ $user->id }}, '{{ addslashes($user->name) }}', '{{ $user->email }}')"
                                                title="Send login link">
                                                🔗 Login Link
                                            </button>
                                            <button class="action-btn btn-green"
                                                onclick="openChatLinkModal({{ $user->id }}, '{{ addslashes($user->name) }}', '{{ $user->email }}')"
                                                title="Send chat link">
                                                💬 Chat Link
                                            </button>
                                        @elseif($user->role === 'agent')
                                            <button class="action-btn btn-blue"
                                                onclick="openAgentSessionsModal({{ $user->id }}, '{{ addslashes($user->name) }}')">
                                                💬 View Chats
                                            </button>
                                        @endif
